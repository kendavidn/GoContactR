...
guinea_contacts_actifs2 <- guinea_contacts_actifs %>%
  gather("date_suivi", "etat_suivi", 27:47)
...
tab_contacts_suivis_complet$etat_suivi[is.na(tab_contacts_suivis_complet$etat_suivi)] <- "Non_vu"
...
epi_suivi_journalier <- ggplot(tab_contacts_suivis_complet, aes(x = date_suivi2, y = n, fill = etat_suivi)) +
  geom_col() +
...
tab_contacts_suivi_jour2 <- contacts_guinea_actifs_ok %>%
  filter(etat_suivi != "Doublon") %>%
  filter(date_suivi2 <= date_revu) %>%
  select(date_suivi2, sous_prefecture, etat_suivi) %>%
  count(date_suivi2, sous_prefecture, etat_suivi)

epi_suivi_journalier2 <- ggplot(tab_contacts_suivi_jour2, aes(x = date_suivi2, y = n, fill = etat_suivi)) +
  geom_col() +
  scale_x_date(date_breaks = "1 day", date_minor_breaks = "1 day", date_labels = "%d%m") +
  scale_fill_manual(" ",
    values = c("Vu" = "#618685", "Non_vu" = "#d64161", "Deplace" = "#4040a1", "Recycle" = "#50394c", "Suspect" = "#ffef96")
...
contacts_guinea_actifs_ok$etat_suivi <-
  recode_factor(
    contacts_guinea_actifs_ok$etat_suivi,
    "Vu" = "Vu",
    "Non_vu" = "Non_vu",
...
Tab_suivis_las24H <- contacts_guinea_actifs_ok %>%
  ...
  select(sous_prefecture, etat_suivi) %>%
  count(sous_prefecture, etat_suivi) %>%
  spread(etat_suivi, n, fill = 0) %>%
  mutate(Vu_24H = Vu) %>%
...
Tab_suivis_non_vus_las48H <- contacts_guinea_actifs_ok %>%
  ...
  select(id_contact, sous_prefecture, etat_suivi) %>%
  count(id_contact, sous_prefecture, etat_suivi) %>%
  spread(etat_suivi, n, fill = 0)


Tab_suivis_non_vus_las48H$etat_suivi2 <- ifelse(Tab_suivis_non_vus_las48H$Non_vu == "2", "Non_vu_last2d", "Vu_last2d")

Tab_suivis_vus_las48H_V2 <- Tab_suivis_non_vus_las48H %>%
  select(sous_prefecture, etat_suivi2) %>%
  count(sous_prefecture, etat_suivi2) %>%
  spread(etat_suivi2, n, fill = 0) %>%
...
Tab_suivis_non_vus_las3d$etat_suivi3 <- ifelse(Tab_suivis_non_vus_las3d$Non_vu == "3", "Non_vu_last3d", "Vu_last3d")

Tab_suivis_vus_las3d_V2 <- Tab_suivis_non_vus_las3d %>%
  select(sous_prefecture, etat_suivi3) %>%
  count(sous_prefecture, etat_suivi3) %>%
  spread(etat_suivi3, n, fill = 0) %>%
