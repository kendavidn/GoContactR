...
contacts_df <-
  ...
  # No follow_up_state column to clean yet because I have not created it
...
contacts_df_long <- 
 contacts_df %>%
  ...
  mutate(follow_up_state = as.character(follow_up_state)) %>%
  # change status of records that should be still active
  mutate(follow_up_state = if_else(follow_up_date > todays_date,
                                           "Suivi futur",
                                           follow_up_state
  )) %>%
  mutate(follow_up_state = replace_na(follow_up_state, "Données manquantes")) %>% 
  # - if follow-up lasted the full 21 days,
  # - change last follow_up state to "Fin de suivi"
  mutate(follow_up_state = if_else(follow_up_day == 21 & follow_up_state == "Vu",
                                           "Fin de suivi",
                                           follow_up_state)) %>% 
  # add legend colors
  left_join(legend_df, by = c("follow_up_state" = "breaks")) %>% 
  ...
  mutate(follow_up_state_simple = follow_up_state) %>% 
  mutate(follow_up_state_simple = recode(follow_up_state_simple,
                                    "Vu" = "Vu",
                                    "Non vu" = "Non vu",
                                    "Cas suspect" = "Vu",
                                    "Decedé" = "Vu",
...
data_to_plot <-
  ...
  mutate(hc_ttip = glue("<b>ID: </b> {id_contact} 
                         ...
                         <b>Status:</b> {follow_up_state}
                         ")) %>%
  ungroup() %>%
  arrange(follow_up_state) %>%   # arranging is necessary so that that colors are pulled in the right order for highcharter
...
data_to_tabulate %>% 
  select(ID = id_contact, 
         Date = follow_up_date, 
         `Follow-up State` = follow_up_state) %>% 
  group_by(ID) %>% 
  arrange(desc(Date)) %>% 
  ungroup() %>% 
...
data_to_plot %>%
  hchart("scatter",
                hcaes(x = follow_up_date, y = row_id,
                      color = colors, group = follow_up_state)) %>%
...
data_to_plot <-
  ...
  count(follow_up_state) %>%
  ungroup() %>%
  complete(follow_up_date, follow_up_state, fill = list(n = 0)) %>%
...
data_to_plot %>% 
  select(1:3) %>% 
  pivot_wider(names_from = follow_up_state, values_from = n) %>% 
...
data_to_tabulate <- 
  data_to_plot %>% 
  select(id_contact, follow_up_date, follow_up_state)
```
...
data_to_plot <- 
  contacts_df_long %>%
  group_by(follow_up_date) %>% 
  count(follow_up_state_simple) %>% 
  ungroup() %>% 
  complete(follow_up_date, follow_up_state_simple, fill = list(n = 0)) %>% 
  # replace 0s with NA for Upcoming follow ups before today's date
  # and replace 0s with NA for past follow ups after today's date
  mutate(n = ifelse(follow_up_state_simple == "Suivi futur" & 
                      follow_up_date <= todays_date, 
                    NA_integer_, 
                    n)) %>% 
  mutate(prop = n/sum(n)) %>% 
  mutate(hc_ttip = glue("<b>Date:</b> {format.Date(follow_up_date,  format = '%a %b %d, %Y')}
                        <b>{follow_up_state_simple}:</b> {n}
                        ")) %>% 
  left_join(legend_df, by = c("follow_up_state_simple" = "breaks")) %>% 
  arrange(follow_up_state_simple)  # arranging is necessary so that that colors are pulled in the right order for highcharter

data_to_plot %>%  
  hchart("column", hcaes(x = follow_up_date, y = n, 
                       color = colors,
                       group = follow_up_state_simple), 
           fillOpacity = 0.35) %>% 
  hc_colors(unique(data_to_plot$colors)) %>% 
  hc_plotOptions(column = list(series = list(fillOpacity = 0.1),
                                   marker = list(enabled = TRUE, 
...
data_to_plot <- 
  contacts_df_long %>%
  filter(follow_up_state_simple != "Suivi futur") %>% 
  group_by(follow_up_date) %>% 
  count(follow_up_state_simple) %>% 
  ungroup() %>% 
  complete(follow_up_date, follow_up_state_simple, fill = list(n = 0)) %>% 
  ...
  ungroup() %>% 
  filter(follow_up_state_simple == "Vu") %>% 
  mutate(hc_ttip = glue("<b>Date:</b> {format.Date(follow_up_date,  format = '%a %b %d, %Y')}
                        <b>{follow_up_state_simple}:</b> {pct_paste} ({n} of {total} )
                        ")) %>% 
  left_join(legend_df, by = c("follow_up_state_simple" = "breaks")) %>% 
  arrange(follow_up_state_simple)  # arranging is necessary so that that colors are pulled in the right order for highcharter

data_to_plot %>%  
  hchart("line", hcaes(x = follow_up_date, y = pct, color = colors, group = follow_up_state_simple), 
           fillOpacity = 0.35) %>% 
  hc_colors(unique(data_to_plot$colors)) %>% 
...
<!--   mutate(follow_up_state = recode(follow_up_state,  -->
...
<!--   filter(follow_up_state != "Missing status") %>%  -->

<!--   group_by(follow_up_date) %>%  -->

<!--   count(follow_up_state) %>%  -->

<!--   #filter(follow_up_state %in% c("Followed", "Not followed")) %>%  -->
...
<!-- #  mutate(n = ifelse(follow_up_state %in% c("Followed", "Not followed") &  -->
...
<!--                         <b>Contacts {str_to_lower(follow_up_state)}:</b> {n} -->

<!--                         ")) %>%  -->

<!--   #inner_join(legend_df, by = c("follow_up_state" = "breaks")) %>%  -->
...
<!--   complete(follow_up_date, prefecture, follow_up_state, fill = list(n = 0)) %>%  -->
