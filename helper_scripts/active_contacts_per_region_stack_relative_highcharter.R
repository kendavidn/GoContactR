...
active_contacts_per_region_stack_relative_highcharter <- 
...
data_to_plot <- 
  ...
  complete(follow_up_date, admin1_contact, fill = list(n = 0, prop = 0)) %>% 
...
data_to_plot %>% 
  hchart("column", hcaes(x = follow_up_date, y = prop, group = admin1_contact)) %>% 
