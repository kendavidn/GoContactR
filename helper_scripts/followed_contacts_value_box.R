...
followed_contacts_value_box <- function(contacts_df, contacts_df_long){
  ...
  data_to_plot <- 
    ...
    filter(follow_up_state_imputed == "Followed") %>% 
    # last 28 days
  ...
  highchart_to_plot <- 
    data_to_plot %>% 
    hchart("area", hcaes(x = follow_up_date, y = n), name = "Followed contacts") %>% 
    hc_size(height = 100) %>% 
  ...
  value_box_out <- valueBoxSpark(
    ...
    title = toupper("Followed contacts, last 4 weeks"),
    sparkobj = highchart_to_plot,
    subtitle = tagList(HTML("&uarr;"), "XX% last 2 weeks"),
    info = "The number of followed contacts reported in the last 28 days",
