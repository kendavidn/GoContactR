...
read_file <- reactive({
  ...
  contacts_df <-
    ...
    filter(date_enreg <= todays_date_reactive()) %>%
  ...
  # send list of output to global environment.
...
#     green,
#     navy,
#     teal, # your friend's name 
#     olive, # my sister's name
#     lime,
#     orange,
#     fuchsia,
#     purple,
...
contacts_registered_per_day_value_box <- 
  function(contacts_df_long){
      ...
      highchart_to_plot <- 
        data_to_plot %>% 
        hchart("column", hcaes(x = date_enreg, y = n), name = "Contacts registered") %>% 
        hc_size(height = 85) %>% 
...
cumulative_contacts_registered_value_box <- 
  function(contacts_df_long){
    ...
    highchart_to_plot <- 
      data_to_plot %>% 
      hchart("column", hcaes(x = date_enreg, y = cum_n), name = "Cumulative contacts registered") %>% 
      hc_size(height = 85) %>% 
      hc_credits(enabled = FALSE) %>% 
      hc_add_theme(hc_theme_sparkline_vb()) 
...
contacts_under_surveillance_value_box <- 
  function(contacts_df_long){
    ...
    highchart_to_plot <- 
      data_to_plot %>% 
      hchart("column", hcaes(x = follow_up_date, y = n), name = "N. under surveillance:") %>% 
      hc_size(height = 85) %>% 
      hc_credits(enabled = FALSE) %>% 
      hc_add_theme(hc_theme_sparkline_vb()) 
    ...
    valueBoxSpark(
      ...
      title = toupper(glue("N. under surveillance")),
      sparkobj = highchart_to_plot,
  ...
  function(contacts_df_long){
   ...
   number_to_sample <- 50
    ...
    data_to_plot <-
      contacts_df_long %>%
      filter(etat_suivi != "Doublon") %>% 
      filter(row_id %in% c(input$snake_plot_slider[1]:input$snake_plot_slider[2]) ) %>% 
      group_by(row_id) %>% 
      # sample
      {if (length(unique(.$row_id)) > number_to_sample) {
      filter(., row_id %in% sample(unique(.$row_id), number_to_sample, replace = F))
      } else .} %>%
      mutate(hc_ttip = glue("<b>Name: </b> {noms234animals
                            _du_contact} <br>
