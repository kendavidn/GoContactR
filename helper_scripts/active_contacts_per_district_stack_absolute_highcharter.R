...
active_contacts_per_district_stack_absolute_highcharter <- 
...
map(unique(data_to_plot$admin1_contact), function(x){
  ...
  data_to_plot %>% 
    ...
    mutate(total = sum(n)) %>% 
    ungroup() %>% 
    mutate(admin2_contact = fct_rev(fct_reorder(admin2_contact, total))) %>% 
    hchart("column", hcaes(x = follow_up_date, y = n, group = admin2_contact)) %>% 
    hc_title(text = x) %>% 
